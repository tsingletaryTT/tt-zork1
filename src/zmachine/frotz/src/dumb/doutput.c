...
void os_display_string (const zchar *s)
{
	...
	/* LLM: Capture output for context (convert to UTF-8 string first) */
#ifdef BUILD_NATIVE
	/* Simple approach: convert zchar string to regular char string */
	static char capture_buf[2048];
	size_t buf_pos = 0;

	const zchar *temp_s = s;
	while ((c = *temp_s++) != 0 && buf_pos < sizeof(capture_buf) - 1) {
		if (c != ZC_NEW_FONT && c != ZC_NEW_STYLE) {
			if (c >= 32 && c <= 126) {
				capture_buf[buf_pos++] = (char)c;
			} else if (c == ZC_GAP || c == ZC_INDENT) {
				capture_buf[buf_pos++] = ' ';
			} else if (c == '\n') {
				capture_buf[buf_pos++] = '\n';
			}
		} else if (c == ZC_NEW_FONT || c == ZC_NEW_STYLE) {
			temp_s++; /* Skip next character */
		}
	}
	capture_buf[buf_pos] = '\0';
