...
/**
 * zork_object_names_safe.cpp - Decode object names with safety checks!
 * Based on debug output showing valid property tables.
 */
...
static uint32_t recursion_depth = 0;  // Track abbreviation recursion
...
// SAFE decoder with recursion limit
static void decode_text_safe(zword z_addr, uint32_t max_recursion) {
    if (recursion_depth >= max_recursion) return;  // Stop deep recursion!
    if (z_addr >= 86000) return;  // Bounds check!
    ...
    do {
        if (words_read++ >= 50) break;  // Safety: max 50 words per string
        if (addr >= 86000) break;
        ...
        for (int i = 10; i >= 0; i -= 5) {
            ...
            case 0:
                if (shift_state == 2 && c == 6)
                    ...
                    outchar(' ');
                else if (z_header.version >= 3 && c >= 1 && c <= 3)
            ...
            case 1:  // abbreviation
                {
                    ...
                    if (ptr_addr < 85000) {  // Safety check!
                        zword abbr_addr = (story_data[ptr_addr] << 8) | story_data[ptr_addr + 1];
                        abbr_addr *= 2;
                        if (abbr_addr < 85000) {
                            decode_text_safe(abbr_addr, max_recursion);
                        }
                    }
                    status = 0;
                }
...
void kernel_main() {
    ...
    const char* header = "=== ZORK OBJECT NAMES (SAFE)! ===\n\n";
    ...
    zword obj_defaults_end = z_header.object_table + 62;
    zword obj_entry = obj_defaults_end;

    msg = "First 30 object names:\n\n";
    while (*msg) output_buffer[output_pos++] = *msg++;
    ...
    for (uint32_t obj_num = 1; obj_num <= MAX_OBJECTS; obj_num++) {
        ...
        if (text_len > 0 && text_len < 20) {
            ...
            // Decode with safety!
            recursion_depth = 0;
            decode_text_safe(name_addr, 5);  // Max 5 levels of abbreviation
